...
require_once 'class-k9-genetics-api.php';
...
class Theme_Manager
{
    ...
    private function theme_hooks()
    {
        ...
        add_filter('walker_nav_menu_start_el', [$this, 'sub_menu_arrow'], 10, 4);
    }
    ...
    {
        if (isset($_REQUEST['custom_action']) && $_REQUEST['custom_action']) {
    ...
    public function import_posts()
    {
        ...
        $response = $this->curl_request('http://old.petlandflorida.com/wp-json/wp/v2/posts', 'GET');
        ...
        if ($header_items['X-WP-TotalPages'] > 1) {
            ...
            for ($page = 2; $page <= $total_pages; $page++) {
                $page_response = $this->curl_request(
                    "http://old.petlandflorida.com/wp-json/wp/v2/posts?page={$page}",
                    'GET'
                );
        ...
        foreach ($posts as $post) {
            ...
            $posts_info[] = [
                ...
                'categories' => $has_categories
                    ? $this->get_post_categories($post->categories)
                    : [],
    ...
    {
        $categories = $this->curl_request(
            'http://old.petlandflorida.com/wp-json/wp/v2/categories?per_page=100',
            'GET'
        );
    ...
    {
        $tags = $this->curl_request(
            'http://old.petlandflorida.com/wp-json/wp/v2/tags?per_page=100',
            'GET'
        );
    ...
    private function get_post_image($media_id)
    {
        $response = $this->curl_request(
            "http://old.petlandflorida.com/wp-json/wp/v2/media/{$media_id}",
            'GET'
        );
    ...
    public function get_available_puppies_page() {
    ...
    public function is_available_puppies_page_template()
    {
        return $this->get_sanitized_page_template_name() === 'templatespuppies-php';
    }

    private function get_sanitized_page_template_name()
    {
        return sanitize_html_class(str_replace('.', '-', get_page_template_slug()));
    }
     ...
     */
    function sub_menu_arrow($item_output, $item)
    {
        if (in_array('menu-item-has-children', $item->classes)) {
            $arrow = '<i class="mainNav__submenu-arrow icon icon--chevron-up"></i>';
    ...
    private function enqueue_styles()
    {
        ...
        wp_register_style(
            'theme-style',
            $this->get_theme_url('styles/dist/style.css'),
            null,
            '2024-06-10'
        );
    ...
    private function enqueue_scripts()
    {
        ...
        wp_register_script(
            'jquery',
            $this->get_theme_url('assets/js/vendor/jquery-3.2.1.slim.min.js'),
            null,
            null,
            true
        );
        ...
        wp_register_script(
            'popper-js',
            $this->get_theme_url('assets/js/vendor/popper.min.js'),
            ['jquery'],
            null,
            true
        );
        ...
        wp_register_script(
            'bootstrap-js',
            $this->get_theme_url('assets/js/vendor/bootstrap.min.js'),
            ['jquery'],
            null,
            true
        );
        ...
        wp_register_script(
            'scrollreveal-js',
            $this->get_theme_url('assets/js/vendor/scrollreveal.min.js'),
            ['jquery'],
            null,
            true
        );
        ...
        wp_register_script(
            'main-script-js',
            $this->get_theme_url('assets/js/main.js'),
            ['jquery'],
            '2018-07-22',
            true
        );
        ...
        wp_register_script(
            'theme-script-js',
            $this->get_theme_url('assets/js/front-script.js'),
            ['jquery'],
            '2022-01-16',
            true
        );
    ...
    public function get_footer_locations()
    {
        ...
        foreach ($locations_raw as $location) {
            ...
            $locations[] = [
                ...
                'phone' => isset($exploded_value[2]) ? $exploded_value[2] : '',
                'days' => isset($exploded_value[3]) ? $exploded_value[3] : '',
                'hours' => isset($exploded_value[4]) ? $exploded_value[4] : '',
                'Facebook' => isset($exploded_value[5]) ? $exploded_value[5] : '',
                'Pinterest' => isset($exploded_value[6]) ? $exploded_value[6] : '',
                'Instagram' => isset($exploded_value[7]) ? $exploded_value[7] : '',
                'Youtube' => isset($exploded_value[8]) ? $exploded_value[8] : '',
                'Twitter' => isset($exploded_value[9]) ? $exploded_value[9] : 'https://twitter.com/petlandflorida',
                'Tiktok' => isset($exploded_value[10]) ? $exploded_value[10] : '',
                'permit_number' => isset($exploded_value[11]) ? $exploded_value[11] : ''
            ];
        }
     ...
     * @return WP_Query
     */
    public function find_posts_by_post_type(
        $post_type,
        $posts_per_page = null,
