...
class Theme_Rest_Api {
	...
	public function post_endpoints() {
		...
		register_rest_route('petland/v1', '/request-puppy-inquiry-information', [
			'methods' => 'POST',
			'callback' => [$this, 'request_puppy_inquiry_information']
		]);
	...
	public function get_available_puppies() {
		return [
			'puppies' => $this->get_petkey_puppies(),
			'breeds' => $this->get_petkey_breeds(),
		];
	}

	public function get_petkey_puppies() {
		$cached_info = get_transient('petkey_puppies');
		if (false === $cached_info) {
			$cached_info = Petkey_Api::get_instance()->get_pet_inventory();

			set_transient('petkey_puppies', $cached_info, 5 * MINUTE_IN_SECONDS);
		}

		return $cached_info;
	}

	public function get_petkey_breeds() {
		$cached_info = get_transient('petkey_breeds');
		if (false === $cached_info) {
			$cached_info = Petkey_Api::get_instance()->get_breed_list();

			set_transient('petkey_breeds', $cached_info, 5 * MINUTE_IN_SECONDS);
		}

		return $cached_info;
	}

	public function get_k9_breeds() {
		$cached_info = get_transient('k9_breeds');
		if (false === $cached_info) {
			$cached_info = K9_Genetics_Api::get_instance()->get_breed_list();

			set_transient('k9_breeds', $cached_info, 5 * MINUTE_IN_SECONDS);
		}
	...
	public function request_puppy_inquiry_information(WP_REST_Request $request) {
		$params = $request->get_json_params();
		...
		$data = [
			...
			'Email' => $params['email'],
			'Mobile' => $params['phone'],
			'Facetime Appointment ?' => $params['faceTimeAppointment'],
			'Message' => $params['message'],
			'Store' => $params['store'],
			'Puppy_ID' => $params['petId'],
			'Puppy_URL' => $params['petUrl'],
	...
	private function create_zoho_lead($data) {
		try {
            ...
            $zoho_lar_id = Theme_Manager::get_instance()->get_theme_option('zoho_lar_id');

            if ($zoho_lar_id) {
                $record->setProperty('lar_id', $zoho_lar_id);
            }
