...
class ReactJs_Templates
{
    ...
    public function enqueue_scripts()
    {
        ...
        $script_vars = [
            'blog_page' => get_query_var( 'paged' ),
            'theme_url' => $this->theme_url,
            'thank_you_page_url' => Theme_Manager::get_instance()->get_theme_page_link(
                'thank_you_page'
            ),
            'privacy_policy_page_url' => Theme_Manager::get_instance()->get_theme_page_link(
                'privacy_policy_page'
            ),
            'terms_and_conditions_page_url' => Theme_Manager::get_instance()->get_theme_page_link(
            ...
            'footer_locations' => Theme_Manager::get_instance()->get_footer_locations(),
            'wp_rest_url' => get_rest_url(null, 'wp/v2'),
            'i18n' => [
                ...
                'all_genders' => __('Any Gender', THEME_LOCALE),
                'age' => __('Age', THEME_LOCALE),
                'sortBy' => __('Sort by', THEME_LOCALE),
            ],
            'is_facebook_plugin_active' => is_plugin_active('official-facebook-pixel/facebook-for-wordpress.php'),
            'facebook_open_bridge_url' => home_url('open-bridge/events')
        ];
        ...
        $script_vars['newsletter_options'] = [
            'newsletter_token' => Theme_Manager::get_instance()->get_theme_option('newsletter_token'),
            'newsletter_list_memberships' => Theme_Manager::get_instance()->get_theme_option('newsletter_list_memberships'),
            'newsletter_title' => Theme_Manager::get_instance()->get_theme_option('newsletter_title'),
            'newsletter_small_title' => Theme_Manager::get_instance()->get_theme_option('newsletter_small_title'),
            'newsletter_bg_image' => Theme_Manager::get_instance()->get_theme_option('newsletter_bg_image'),
            'newsletter_bg_mobile_image' => Theme_Manager::get_instance()->get_theme_option('newsletter_bg_mobile_image'),
        ];
     ...
     * @return array
     */
    private function get_pet_ages()
    {
        $pet_ages = Theme_Manager::get_instance()->get_theme_option('pet_ages');
        $ages = [];

        if (is_array($pet_ages) && !empty($pet_ages)) {
            foreach ($pet_ages as $pet_age) {
                $explode_value = explode('|', $pet_age);
                $ages[] = [
                    'name' => $explode_value[1],
                    'value' => $explode_value[0]
                ];
